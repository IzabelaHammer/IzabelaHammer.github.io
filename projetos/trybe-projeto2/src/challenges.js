...
function highestCount(numbers) {
  ...
  let arrBigNumber = numbers.filter((number) => number === maior);
  // parte comentada é para mostrar como estava o codigo: let count = 0;
  /* for (let i = 0; i < numbers.length; i += 1) {
    if (numbers[i] === maior) {
      count += 1;
    }
  } */
  return arrBigNumber.length;
}

// Desafio 7
function catAndMouse(mouse, cat1, cat2) {
  // seu código aqui
  let conta1 = Math.abs(mouse - cat1); // modulo
...
function fizzBuzz(vetornumbers) {
  // seu código aqui
  let vetorfizzBuzz = vetornumbers.map((number) => testfizzBuzz(number));
